...
export default async function ReportsPage(){
  ...
  if (!roleAllowed(ctx.role, ROLE_GROUPS.reports)) return <AccessDenied area="reports" />;
  const since30=new Date(Date.now()-30*86400000).toISOString();
  const today=dateInTimeZone(new Date(),ctx.organization.timezone);
  const canReadLeads=roleAllowed(ctx.role, ROLE_GROUPS.retention);
  const [activeMemberships,attendance,payments,leads,renewals]=await Promise.all([
    ...
    canReadLeads ? ctx.supabase.from("leads").select("id,stage,source").eq("organization_id",ctx.organization.id).gte("created_at",since30) : Promise.resolve({data:[] as any[],error:null}),
