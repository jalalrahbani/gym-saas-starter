...
export default async function DashboardPage() {
  ...
  const canViewFinancial = roleAllowed(ctx.role, ROLE_GROUPS.financial);
  const canManageRetention = roleAllowed(ctx.role, ROLE_GROUPS.retention);
  const now = new Date();
  const todayDate = dateInTimeZone(now, ctx.organization.timezone);
  const [year, month] = todayDate.split("-");
  const startMonth = wallTimeToUtcIso(`${year}-${month}-01T00:00`, ctx.organization.timezone);
  ...
  if (canViewFinancial) {
    for (const payment of paymentsRes.data ?? []) {
      revenueByCurrency.set(payment.currency, (revenueByCurrency.get(payment.currency) ?? 0) + Number(payment.amount_minor));
    }
  }
  const revenueText = revenueByCurrency.size === 0
    ? formatMoney(0, ctx.organization.base_currency)
    : [...revenueByCurrency.entries()].map(([currency, amount]) => formatMoney(amount, currency)).join(" · ");
  ...
  const engagement = (memberProfilesRes.data ?? []).map((member: any) => ({
    member,
  ...
  if (canViewFinancial) metrics.push(["Revenue this month", revenueText, "Paid transactions only"]);
  metrics.push(["Renewals this week", String(expiringRes.data?.length ?? 0), canManageRetention ? "WhatsApp-ready renewal queue" : "Membership expiries"]);

  return <section className="mx-auto max-w-7xl">
    <div className="mb-7"><p className="text-sm text-[#7a7f89]">{new Intl.DateTimeFormat(undefined, { weekday: "long", day: "numeric", month: "long", timeZone: ctx.organization.timezone }).format(now)}</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Here’s what needs attention.</h1><p className="mt-2 text-sm text-[#7a7f89]">The dashboard only presents financial and outreach actions when your staff role is authorized to use them.</p></div>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
      <section className="rounded-2xl border border-[#e4e6ea] bg-white">
        <div className="flex items-center justify-between border-b border-[#eceef1] p-5"><div><h2 className="font-semibold">Renewal queue</h2><p className="mt-1 text-sm text-[#7a7f89]">Upcoming membership expiries.</p></div>{canManageRetention&&<Link href="/messages" className="text-sm font-semibold">Retention center →</Link>}</div>
        <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="bg-[#fafafa] text-xs uppercase text-[#8a9099]"><tr><th className="px-5 py-3">Member</th><th>Plan</th><th>Expires</th>{canViewFinancial&&<th>Value</th>}<th>Action</th></tr></thead><tbody>{(expiringRes.data??[]).map((row:any)=>{
          ...
          const wa=canManageRetention?whatsappHref(member?.phone,text):null;
          return <tr key={row.id} className="border-t border-[#f0f1f3]"><td className="px-5 py-4"><p className="font-medium">{member?.first_name} {member?.last_name}</p><p className="mt-1 text-xs text-[#8a9099]">{member?.phone||"No phone"}</p></td><td>{row.membership_plans?.name??"—"}</td><td>{formatDate(row.ends_on)}</td>{canViewFinancial&&<td>{formatMoney(row.price_minor,row.currency)}</td>}<td className="pr-5">{canManageRetention?(wa?<a href={wa} target="_blank" rel="noreferrer" className="rounded-lg bg-[#111318] px-3 py-1.5 text-xs font-semibold text-white">WhatsApp renewal</a>:<Link href={`/members/${row.member_id}`} className="rounded-lg border border-[#dfe2e7] px-3 py-1.5 text-xs font-semibold">Add phone</Link>):<Link href={`/members/${row.member_id}`} className="text-xs font-semibold">Open member →</Link>}</td></tr>;
        })}</tbody></table></div>
    ...
    <section className="mt-6 rounded-2xl border border-[#e4e6ea] bg-white"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#eceef1] p-5"><div><h2 className="font-semibold">Retention pulse</h2><p className="mt-1 text-sm text-[#7a7f89]">Membership validity alone does not tell you whether members are engaged.</p></div>{canManageRetention&&<Link href="/messages" className="rounded-lg border border-[#dfe2e7] px-3 py-2 text-xs font-semibold">Open engagement queues</Link>}</div><div className="grid gap-0 divide-y divide-[#f0f1f3] sm:grid-cols-3 sm:divide-x sm:divide-y-0"><div className="p-5"><p className="text-sm font-medium">High engagement</p><p className="mt-2 text-3xl font-semibold text-emerald-700">{streakCount}</p><p className="mt-2 text-xs text-[#7a7f89]">Members currently on a 5+ consecutive-day streak.</p></div><div className="p-5"><p className="text-sm font-medium">At risk</p><p className="mt-2 text-3xl font-semibold text-amber-700">{atRiskCount}</p><p className="mt-2 text-xs text-[#7a7f89]">No visit for 8–30 days.</p></div><div className="p-5"><p className="text-sm font-medium">Inactive</p><p className="mt-2 text-3xl font-semibold text-rose-700">{inactiveCount}</p><p className="mt-2 text-xs text-[#7a7f89]">No visit for 30+ days or no attendance history.</p></div></div></section>
  </section>;
}
