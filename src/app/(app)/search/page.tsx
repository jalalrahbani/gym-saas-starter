...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const canSearchLeads = roleAllowed(ctx.role, ROLE_GROUPS.retention);
  const canSearchPayments = roleAllowed(ctx.role, ROLE_GROUPS.financial);

  if (!q) return <section className="mx-auto max-w-5xl"><h1 className="text-3xl font-semibold">Search</h1><p className="mt-2 text-[#7a7f89]">Enter a member name, phone or member number{canSearchLeads ? ", lead name" : ""}{canSearchPayments ? " or receipt number" : ""}.</p></section>;
  ...
  else membersQuery = membersQuery.or(`first_name.ilike.%${safeQ}%,last_name.ilike.%${safeQ}%,phone.ilike.%${safeQ}%`);

  const [membersRes, leadsRes, paymentsRes] = await Promise.all([
    membersQuery,
    canSearchLeads
      ? ctx.supabase.from("leads").select("id,full_name,phone,stage,converted_member_id").eq("organization_id", ctx.organization.id).or(`full_name.ilike.%${safeQ}%,phone.ilike.%${safeQ}%`).limit(12)
      : Promise.resolve({ data: [] as any[], error: null }),
    canSearchPayments && /^\d+$/.test(q)
  ...
  return <section className="mx-auto max-w-5xl space-y-6"><div><p className="text-sm text-[#7a7f89]">Workspace search</p><h1 className="mt-1 text-3xl font-semibold">Results for “{q}”</h1><p className="mt-2 text-sm text-[#7a7f89]">{count} matching record{count === 1 ? "" : "s"} available to your role</p></div>
