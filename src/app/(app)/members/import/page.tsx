...
export default async function MemberImportPage({ searchParams }: { searchParams: Promise<Record<string,string|string[]|undefined>> }) {
  ...
  if (!roleAllowed(ctx.role, ROLE_GROUPS.memberManagers)) return <AccessDenied area="member import" />;
  ...
  return <section className="mx-auto max-w-3xl space-y-6">
    ...
    <section className="rounded-2xl border border-[#e4e6ea] bg-white p-6"><form action={importMembersCsvAction} className="space-y-4"><OperationKeyInput /><label className="block text-sm font-medium">Home location<select name="home_location_id" defaultValue={ctx.location.id} className="mt-1.5 w-full rounded-xl border border-[#dfe2e7] px-3 py-2.5">{ctx.locations.map((l)=><option key={l.id} value={l.id}>{l.name}</option>)}</select></label><label className="block text-sm font-medium">CSV file<input name="file" type="file" accept=".csv,text/csv" required className="mt-1.5 block w-full rounded-xl border border-[#dfe2e7] bg-white px-3 py-2.5 text-sm"/></label><button data-feedback="Validating import…" className="w-full rounded-xl bg-[#111318] px-4 py-3 text-sm font-semibold text-white">Validate & import members</button></form></section>
  </section>;
}
