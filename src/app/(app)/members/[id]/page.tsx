...
export default async function MemberDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [memberRes, plansRes, membershipsRes, paymentsRes, attendanceRes, attendanceCountRes, notesRes, credentialsRes] = await Promise.all([
    ...
    ctx.supabase.from("payments").select("id,receipt_number,amount_minor,currency,payment_method,status,paid_at,membership_id").eq("organization_id", ctx.organization.id).eq("member_id", id).order("paid_at", { ascending: false }).limit(150),
    ...
    ctx.supabase.from("member_notes").select("id,note,created_at").eq("organization_id", ctx.organization.id).eq("member_id", id).order("created_at", { ascending: false }).limit(50),
    ctx.supabase.from("access_credentials").select("id,credential_type,last_four,label,is_active,issued_at,revoked_at").eq("organization_id", ctx.organization.id).eq("member_id", id).order("issued_at", { ascending: false }),
  ]);
  ...
  const whatsapp = whatsappHref(member.phone, renewalMessage);
  ...
  for (const membership of memberships) {
    timeline.push({
      ...
      detail: `${formatDate(membership.starts_on)} → ${formatDate(membership.ends_on)} · ${formatMoney(membership.price_minor, membership.currency)}`,
      kind: "membership",
    });
      ...
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="mt-2"><PhotoUploader organizationId={ctx.organization.id} memberId={id} /></div>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          {whatsapp ? <a href={whatsapp} target="_blank" rel="noreferrer" className="rounded-xl bg-[#111318] px-4 py-2.5 text-sm font-semibold text-white">WhatsApp renewal</a> : <span className="rounded-xl bg-[#f5f6f7] px-4 py-2.5 text-sm text-[#7a7f89]">Add phone with country code</span>}
        </div>
      </div>

      <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <div className="rounded-xl bg-[#f7f8f9] p-4"><p className="text-xs uppercase tracking-wide text-[#8a9099]">Membership</p><span className={`mt-2 inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${membershipStateClass}`}>{membershipState}</span><p className="mt-2 text-xs text-[#7a7f89]">{active?.membership_plans?.name ?? "Assign or renew below"}</p></div>
    ...
    <div className="grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
      <div className="space-y-6">
        <section className="rounded-2xl border border-[#e4e6ea] bg-white p-5">
          <div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold">Member lifecycle</h2><p className="mt-1 text-sm text-[#7a7f89]">One timeline from original join date through memberships, money, access and attendance.</p></div><span className="rounded-full bg-[#f1f3f5] px-3 py-1 text-xs font-semibold">Since {formatDate(member.joined_at)}</span></div>
        ...
        <section className="rounded-2xl border border-[#e4e6ea] bg-white p-5">
          ...
          <p className="mt-1 text-sm text-[#7a7f89]">Use international phone format (for example +961…) so WhatsApp renewal actions work reliably.</p>
          <form action={updateMemberAction} className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <div className="flex items-end"><button className="w-full rounded-lg bg-[#111318] px-4 py-2.5 text-sm font-semibold text-white">Save profile</button></div>
          </form>
        </section>

        <section className="rounded-2xl border border-[#e4e6ea] bg-white">
          <div className="border-b border-[#eceef1] p-5"><h2 className="font-semibold">Membership history</h2><p className="mt-1 text-sm text-[#7a7f89]">Every plan, renewal, freeze, payment balance and visit-pack balance stays attached to this member.</p></div>
          <div className="divide-y divide-[#f0f1f3]">{memberships.map((membership: any) => {
            const linkedPaid = payments.filter((payment: any) => payment.status === "paid" && payment.membership_id === membership.id).reduce((sum: number, payment: any) => sum + Number(payment.amount_minor), 0);
            const balance = Math.max(0, Number(membership.price_minor) - linkedPaid);
            const isFrozenNow = (membership.membership_freezes ?? []).some((freeze: any) => freeze.starts_on <= today && freeze.ends_on >= today);
            return <div key={membership.id} className="p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-medium">{membership.membership_plans?.name ?? "Membership"}</p><p className="mt-1 text-xs text-[#7a7f89]">{formatDate(membership.starts_on)} → {formatDate(membership.ends_on)}{membership.visits_remaining != null ? ` · ${membership.visits_remaining} visits remaining` : ""}</p><p className="mt-1 text-xs text-[#7a7f89]">Paid {formatMoney(linkedPaid, membership.currency)} · Balance {formatMoney(balance, membership.currency)}{isFrozenNow ? " · Frozen now" : ""}</p></div><span className="rounded-full bg-[#f0f2f4] px-2.5 py-1 text-xs font-semibold capitalize">{membership.status}</span></div>{(membership.membership_freezes ?? []).length > 0 && <div className="mt-3 space-y-1">{membership.membership_freezes.map((freeze: any) => <p key={freeze.id} className="text-xs text-[#6f7580]">Freeze: {formatDate(freeze.starts_on)} → {formatDate(freeze.ends_on)}{freeze.reason ? ` · ${freeze.reason}` : ""}</p>)}</div>}{membership.status === "active" && <details className="mt-3"><summary className="cursor-pointer text-xs font-semibold">Freeze / pause membership</summary><form action={freezeMembershipAction} className="mt-3 grid gap-2 sm:grid-cols-4"><input type="hidden" name="member_id" value={id} /><input type="hidden" name="membership_id" value={membership.id} /><input name="starts_on" type="date" min={today} required className="rounded-lg border border-[#dfe2e7] px-2 py-1.5 text-xs" /><input name="ends_on" type="date" min={today} required className="rounded-lg border border-[#dfe2e7] px-2 py-1.5 text-xs" /><input name="reason" placeholder="Reason (optional)" className="rounded-lg border border-[#dfe2e7] px-2 py-1.5 text-xs" /><button className="rounded-lg border border-[#111318] px-2 py-1.5 text-xs font-semibold">Apply freeze</button></form></details>}</div>;
          })}{memberships.length === 0 && <p className="p-5 text-sm text-[#7a7f89]">No memberships yet.</p>}</div>
        </section>

        <section className="rounded-2xl border border-[#e4e6ea] bg-white">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#eceef1] p-5"><div><h2 className="font-semibold">Attendance history</h2><p className="mt-1 text-sm text-[#7a7f89]">Check-in, checkout and visit duration feed the engagement score and streak.</p></div><div className="flex gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${engagementBadgeClass(insight.engagement)}`}>{insight.engagementLabel}</span><span className="rounded-full bg-[#f1f3f5] px-2.5 py-1 text-xs font-semibold">{visitsLast30} visit days / 30d</span></div></div>
          <div className="divide-y divide-[#f0f1f3]">{attendance.slice(0, 60).map((visit: any) => <div key={visit.id} className="flex items-center justify-between gap-4 p-4 text-sm"><div><p className="font-medium">{formatDateTime(visit.checked_in_at, ctx.organization.timezone)}</p><p className="text-xs text-[#7a7f89]">{visit.check_in_method}{visit.forced_closed ? " · auto/forced close" : ""}</p></div><div className="text-right"><p>{visit.checked_out_at ? formatDateTime(visit.checked_out_at, ctx.organization.timezone) : "Currently inside"}</p><p className="text-xs text-[#7a7f89]">{durationBetween(visit.checked_in_at, visit.checked_out_at)}</p></div></div>)}{!attendance.length && <p className="p-5 text-sm text-[#7a7f89]">No visits recorded.</p>}</div>
        </section>
      </div>

      <div className="space-y-6">
        <section className="rounded-2xl border border-[#e4e6ea] bg-white p-5">
          <h2 className="font-semibold">Renewal & contact</h2>
          <div className="mt-4 rounded-xl bg-[#f7f8f9] p-4"><p className="text-sm font-semibold">{active?.membership_plans?.name ?? "Membership renewal required"}</p><p className="mt-1 text-sm text-[#7a7f89]">{active?.ends_on ? `Expires ${formatDate(active.ends_on)}${daysUntilRenewal != null ? ` · ${daysUntilRenewal} days remaining` : ""}` : "No active membership today."}</p></div>
          {whatsapp ? <a href={whatsapp} target="_blank" rel="noreferrer" className="mt-3 flex w-full justify-center rounded-lg bg-[#111318] px-4 py-2.5 text-sm font-semibold text-white">Send renewal on WhatsApp</a> : <p className="mt-3 rounded-lg bg-amber-50 p-3 text-sm text-amber-900">Add the member's phone with country code to enable WhatsApp renewal messaging.</p>}
        </section>
